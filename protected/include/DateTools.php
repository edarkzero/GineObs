...
class DateTools
{
    ...
    const LONG_DATETIME_FORMAT_JS = "dd 'de' MM 'de' yyyy hh:mm A";
    const LONG_TIME_FORMAT_JS = "hh:mm A";
    const BDD_DATETIME_FORMAT_JS = "yyyy-mm-dd hh:ii";
    const BDD_DATE_FORMAT_JS = "yyyy-mm-dd";
    ...
    /**
     * return the formated time 12h to 24h
     *
     * @param $ampmString
     * @return string
     */
    public static function parseAmPmTo24h($ampmString)
    {
        if (strstr($ampmString, 'PM') !== false) {
            $str = explode(':', $ampmString);
            if ($str[0] !== '12') {
                $intH = (int)$str[0];
                $str[0] = (string)($intH + 12);
            }
            $str[1] = trim($str[1], 'PM');
            return trim($str[0] . ':' . $str[1]);
        } elseif (strstr($ampmString, 'AM') !== false) {
            $str = explode(':', $ampmString);
            if ($str[0] === '12')
                $str[0] = '00';
            $str[1] = trim($str[1], 'AM');
